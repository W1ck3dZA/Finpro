export type CsvImportType = "CLIENT" | "JOB" | "TIME";
 ...
 */
export function detectCsvType(headers: string[]): CsvImportType | null {
  const set = new Set(headers.map((h) => h.trim()));

  if (set.has("[State] State") && set.has("[Job] Budget")) return "JOB";
  if (set.has("[Time] Time") && set.has("[Staff] Name")) return "TIME";
  if (set.has("[Client] Business Structure")) return "CLIENT";
  return null;
}
